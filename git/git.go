...
func (gitInstance *git) NewBranch(message string) {
	ctx := context.Background()
	cmdGit := exec.CommandContext(ctx, "git", "checkout", "-b", message)
	output, err := cmdGit.Output()
	if err != nil {
		fmt.Println(color.Red(errors.New("no create new branch")))
		os.Exit(1)
	}
...
func (gitInstance *git) getInnerVariables() map[string]string {
	extra := map[string]string{
		"BRANCH_NAME": gitInstance.BranchName(),
	}
